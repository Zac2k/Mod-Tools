...
public static class StandardShaderConverter
{
    ...
    public static Material GetConvertedMat(Material Mat)
    {
        ...
        )
        {
            ...
            if (mode == 0) ShaderToUse = OpaqueShader;
            else
            if (mode == 1) ShaderToUse = CutoutShader;
            else
            if (mode == 2) ShaderToUse = FadeShader;
            else
            if (mode == 3) ShaderToUse = TransparentShader;
            else
                return Mat;
        }
        else
    ...
    public static Texture2D GetStandardPBR(int Size, Texture2D Occ, Texture2D MetalSpec, Texture2D Emit, Texture2D Diff = null)
    {
        ...
        if (Emit != null) Emit = Emit.GetReadable(Size, Size);
        else
        {
